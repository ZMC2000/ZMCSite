...
type ImageTarget =
  | { type: "hero" }
  | { type: "floorMain"; floorIndex: number }
  | { type: "floorGallery"; floorIndex: number; imageIndex: number };
...
export default function ServicesEditor({
  ...
  function updateHero<K extends keyof ServicesPageData["hero"]>(
    key: K,
    value: ServicesPageData["hero"][K],
  ) {
    setData({
      ...data,
      hero: {
  ...
  function updateHeroImage<K extends keyof ServicesPageData["hero"]["image"]>(
    key: K,
    value: ServicesPageData["hero"]["image"][K],
  ) {
    setData({
      ...data,
      hero: {
  ...
  function updateFloorMainImage(
    ...
    nextFloors[floorIndex] = {
      ...nextFloors[floorIndex],
      mainImage: {
        ...nextFloors[floorIndex].mainImage,
        [key]: value,
      },
    };
  ...
  function updateGalleryImage(
    ...
    const nextFloors = [...data.floors];
    const nextGallery = [...nextFloors[floorIndex].gallery];

    nextGallery[imageIndex] = {
      ...nextGallery[imageIndex],
      [key]: value,
    };
  ...
  function removeFloor(floorIndex: number) {
    setData({
      ...data,
  ...
  function removeGalleryImage(floorIndex: number, imageIndex: number) {
    const nextFloors = [...data.floors];

    nextFloors[floorIndex] = {
      ...nextFloors[floorIndex],
      gallery: nextFloors[floorIndex].gallery.filter(
  ...
  function chooseImage(src: string) {
    ...
    if (imageTarget.type === "floorMain") {
      updateFloorMainImage(imageTarget.floorIndex, "src", cleanedSrc);
    }

    if (imageTarget.type === "floorGallery") {
      updateGalleryImage(
        imageTarget.floorIndex,
  ...
  async function saveChanges() {
    ...
    try {
      ...
      setMessage("Services page updated successfully.");
    } catch (saveError) {
      setError(
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-5">
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <h1 className="text-4xl font-extrabold tracking-tight text-gray-950 sm:text-5xl">
                  Edit Clinics and Services Page
                </h1>

                <p className="mt-4 max-w-2xl text-base font-medium leading-8 text-gray-600">
                  Update the hero content, floor descriptions, main images, and
                  image galleries from the Services page JSON file.
                </p>
              </div>
          ...
          <div className="space-y-10">
            <section className="rounded-4xl border border-gray-100 bg-white p-6 shadow-xl shadow-gray-200/60 sm:p-8">
              <SectionHeader
                title="Hero Section"
                description="Edit the page title, subtitle, and hero banner image."
              />
              ...
              <div className="mb-8 flex flex-col gap-4 border-b border-gray-100 pb-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm font-medium leading-6 text-gray-500">
                    Add, edit, or remove floors and services sections.
                  </p>
                </div>
