...
type ImageTarget =
  | { type: "hero" }
  | { type: "doctor"; categoryIndex: number; doctorIndex: number };
...
export default function ScheduleEditor({
  ...
  const visibleCategories =
    selectedCategory === "All"
      ? data.categories.map((category, index) => ({ category, index }))
      : data.categories
          .map((category, index) => ({ category, index }))
          .filter(({ category }) => category.name === selectedCategory);

  function updateHero<K extends keyof SchedulePageData["hero"]>(
    key: K,
    value: SchedulePageData["hero"][K],
  ) {
    setData({
      ...data,
      hero: {
  ...
  function updateHeroImage<K extends keyof SchedulePageData["hero"]["image"]>(
    key: K,
    value: SchedulePageData["hero"]["image"][K],
  ) {
    setData({
      ...data,
      hero: {
  ...
  function removeCategory(categoryIndex: number) {
    const categoryName = data.categories[categoryIndex].name;

    setData({
      ...data,
      categories: data.categories.filter((_, index) => index !== categoryIndex),
    });
  ...
  function addDoctor(categoryIndex: number) {
    const nextCategories = [...data.categories];
  ...
  function removeDoctor(categoryIndex: number, doctorIndex: number) {
    const nextCategories = [...data.categories];

    nextCategories[categoryIndex] = {
      ...nextCategories[categoryIndex],
      doctors: nextCategories[categoryIndex].doctors.filter(
        (_, index) => index !== doctorIndex,
      ),
    };
  ...
  function updateDoctorTime(
    ...
    const nextCategories = [...data.categories];
    const nextDoctors = [...nextCategories[categoryIndex].doctors];
    const nextTimes = [...nextDoctors[doctorIndex].times];
  ...
  function addDoctorTime(categoryIndex: number, doctorIndex: number) {
    const nextCategories = [...data.categories];
    const nextDoctors = [...nextCategories[categoryIndex].doctors];
  ...
  function removeDoctorTime(
    ...
    nextDoctors[doctorIndex] = {
      ...nextDoctors[doctorIndex],
      times: nextDoctors[doctorIndex].times.filter(
  ...
  function chooseImage(src: string) {
    ...
    if (imageTarget.type === "doctor") {
      updateDoctor(
        imageTarget.categoryIndex,
        imageTarget.doctorIndex,
        "image",
        cleanedSrc,
      );
    }
  ...
  async function saveChanges() {
    ...
    try {
      ...
      setMessage("Schedule page updated successfully.");
    } catch (saveError) {
      setError(
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-5">
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-4 max-w-2xl text-base font-medium leading-8 text-gray-600">
                  Update the schedule hero, categories, doctors, available
                  times, notes, images, and confirmation CTA.
                </p>
              </div>
          ...
          <div className="space-y-10">
            <section className="rounded-4xl border border-gray-100 bg-white p-6 shadow-xl shadow-gray-200/60 sm:p-8">
              <SectionHeader
                title="Hero Section"
                description="Edit the hero title, subtitle, and banner image."
              />
              ...
              <div className="mb-8 flex flex-col gap-5 border-b border-gray-100 pb-6 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm font-medium leading-6 text-gray-500">
                    Filter by category, then edit doctors, images, times, and
                    notes.
                  </p>
                </div>
            ...
            <section className="rounded-4xl border border-gray-100 bg-white p-6 shadow-xl shadow-gray-200/60 sm:p-8">
              <SectionHeader
                title="CTA Section"
                description="Edit the confirmation call to action at the bottom of the schedule page."
              />
